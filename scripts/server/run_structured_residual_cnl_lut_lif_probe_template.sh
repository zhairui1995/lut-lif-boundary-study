...
if [[ -z "$GPU" ]]; then
  echo "[structured-cnl-lut-lif] set GPU or CUDA_VISIBLE_DEVICES" >&2
  exit 2
fi
...
RESULT_DIR="${RESULT_DIR:-$REPO_ROOT/results/structured_residual_cnl_lut_lif_smoke}"
...
if [[ "$RUN_MODE" == "smoke" ]]; then
  ...
  echo "[structured-cnl-lut-lif] RUN_MODE must be smoke or full, got: $RUN_MODE" >&2
  exit 2
fi
