...
from lut_if.neuron import DenseLUTIFNeuron, _bilinear_lookup, _nearest_lookup
...
class StructuredResidualCNLLUTIF(DenseLUTIFNeuron):
    """CNL-LUT-LIF with a low-rank additive residual over a fixed transition table."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        base_table = self.initial_table.detach().clone()
        del self._parameters["transition_table"]
        self.register_buffer("base_transition_table", base_table)
        self.state_residual = torch.nn.Parameter(torch.zeros(self.state_levels, 1))
        self.input_residual = torch.nn.Parameter(torch.zeros(1, self.input_levels))
        self.global_residual = torch.nn.Parameter(torch.zeros(()))
    ...
    def _structured_table(self, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        return (
            self.base_transition_table.to(device=device, dtype=dtype)
            + self.state_residual.to(device=device, dtype=dtype)
            + self.input_residual.to(device=device, dtype=dtype)
            + self.global_residual.to(device=device, dtype=dtype)
        )

    def _lookup(self, v: torch.Tensor, x: torch.Tensor):
        table = self._structured_table(x.device, x.dtype)
        v_range = (
    ...
    def _record_quantization_error(self, v: torch.Tensor, x: torch.Tensor) -> None:
        if self.training:
            return
        with torch.no_grad():
            table = self._structured_table(x.device, x.dtype).detach()
            v_range = (
                self.v_lo.to(device=x.device, dtype=x.dtype),
                self.v_hi.to(device=x.device, dtype=x.dtype),
            )
    ...
    def _normalize_step(self, step: int, current: torch.Tensor, flattened_tb: bool, batch_size: int) -> torch.Tensor:
        ...
        if channel_count != self.ref_mean.shape[1]:
            raise RuntimeError(
                f"channel mismatch in StructuredResidualCNLLUTIF: current has {channel_count}, "
                f"moments have {self.ref_mean.shape[1]}"
            )
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        if x_seq.ndim < 2:
            raise ValueError("StructuredResidualCNLLUTIF expects a time-major tensor")
        self.executions += 1
    ...
    def regularization(self) -> Dict[str, torch.Tensor]:
        state_smooth = (self.state_residual[1:] - self.state_residual[:-1]).square().mean()
        input_smooth = (self.input_residual[:, 1:] - self.input_residual[:, :-1]).square().mean()
        delta_l2 = (
            self.state_residual.square().mean()
            + self.input_residual.square().mean()
            + self.global_residual.square()
        )
        if self.learn_norm_affine:
            norm_affine = (self.norm_scale - 1.0).square().mean() + self.norm_bias.square().mean()
        else:
            norm_affine = self.base_transition_table.new_tensor(0.0)
        return {
            "delta_l2": delta_l2,
            "neighbor_smoothness": state_smooth + input_smooth,
            "norm_affine": norm_affine,
        }

    def table_entries(self) -> int:
        return int(self.base_transition_table.numel())

    def trainable_residual_entries(self) -> int:
        return int(self.state_residual.numel() + self.input_residual.numel() + self.global_residual.numel())

    def metadata_bytes(self) -> int:
        residual_bytes = self.trainable_residual_entries() * self.state_residual.element_size()
        moment_bytes = 4 * self.ref_mean.numel() * 4
        affine_bytes = 2 * self.ref_mean.numel() * 4 if self.learn_norm_affine else 0
        scalar_metadata = 7 * 4
        return int(residual_bytes + moment_bytes + affine_bytes + scalar_metadata)

    def summary(self) -> Dict[str, float]:
        values = max(self.input_count, 1)
        current_values = max(self.current_values, 1)
        state_rms = self.state_residual.detach().square().mean().sqrt()
        input_rms = self.input_residual.detach().square().mean().sqrt()
        delta = self.state_residual.detach() + self.input_residual.detach() + self.global_residual.detach()
        return {
            ...
            "logical_table_entries": float(self.table_entries()),
            "trainable_residual_entries": float(self.trainable_residual_entries()),
            "metadata_bytes": float(self.metadata_bytes()),
            "threshold": float(self.threshold.detach().item()),
            "structured_delta_rms": float(delta.square().mean().sqrt().item()),
            "state_residual_rms": float(state_rms.item()),
            "input_residual_rms": float(input_rms.item()),
            "global_residual": float(self.global_residual.detach().item()),
            ...
            "current_mse_after_cnl": self.current_sse_after / current_values,
        }


def replace_with_structured_residual_cnl_lutif(
    ...
    *,
    ...
    learn_norm_affine: bool,
) -> Dict[str, StructuredResidualCNLLUTIF]:
    replacements: Dict[str, StructuredResidualCNLLUTIF] = {}
    for name, original in list(targets.items()):
        replacement = StructuredResidualCNLLUTIF(
...
def regularization_terms(modules: Mapping[str, StructuredResidualCNLLUTIF]) -> Dict[str, torch.Tensor]:
    terms = [module.regularization() for module in modules.values()]
    if not terms:
        raise ValueError("no structured residual CNL-LUT-LIF modules were provided")
...
def train_one_epoch(teacher, candidate, replacements, loader, optimizer, scaler, device, args, epoch: int):
    ...
    for batch_index, (images, target) in enumerate(loader):
        ...
        if not torch.isfinite(loss):
            raise RuntimeError(f"non-finite structured residual CNL-LUT-LIF loss at epoch={epoch} batch={batch_index}: {loss.item()}")
...
def build_report(metrics: Mapping[str, object]) -> str:
    ...
    return f"""# Structured Residual CNL-LUT-LIF Probe
...
- Fixed {metrics['protocol']['state_bits']}-bit state/input transition grid
...
def run(args):
    ...
    structured_model, structured_checkpoint = build_model(root, args, device)
    for model in (posthoc_model, quant_model, fixed_cnl_model, structured_model):
        freeze(model)
    ...
    structured_replacements = replace_with_structured_residual_cnl_lutif(
        structured_model,
        discover_lif_targets(structured_model),
        ranges,
        moments,
        state_bits=args.state_bits,
        input_bits=args.input_bits,
    ...
    initial = evaluate_pair(clean, structured_model, val_loader, device, structured_replacements, args.max_eval_batches)

    parameters = list(trainable_parameters(structured_replacements))
    if not parameters:
        raise RuntimeError("structured residual CNL-LUT-LIF produced no trainable parameters")
    ...
    for epoch in range(1, args.epochs + 1):
        row = train_one_epoch(clean, structured_model, structured_replacements, train_loader, optimizer, scaler, device, args, epoch)
        validation = evaluate_pair(clean, structured_model, val_loader, device, structured_replacements, args.max_eval_batches)
        ...
        if validation["candidate_top1"] > best_top1:
            ...
            best_state = {
                ...
                for name, module in structured_replacements.items()
            }
    if best_eval is None or best_state is None:
        raise RuntimeError("no best structured residual CNL-LUT-LIF state was recorded")
    torch.save(
        {"protocol": vars(args), "replacements": best_state, "best_evaluation": best_eval},
        result_dir / "structured_residual_cnl_lut_lif_best.pt",
    )

    fixed_cnl_storage = metadata_summary(fixed_cnl_replacements)
    structured_storage = metadata_summary(structured_replacements)
    posthoc_storage = generic_module_summary(posthoc_replacements)
    quant_storage = metadata_summary(quant_replacements)
    expected_targets = len(structured_replacements)
    all_targets_executed = bool(
        ...
        and structured_storage["modules_executed"] == expected_targets
    )
    beats_fixed_cnl = (
    ...
    summary_rows = [
        ...
        {"method": "structured_residual_CNL_LUT_LIF_initial", **initial},
        {"method": "structured_residual_CNL_LUT_LIF_final", **best_eval},
    ]
    metrics = {
        "experiment": "structured_residual_cnl_lut_lif_probe",
        "status": "completed",
        "protocol": {
            "family": args.family,
            "seed": args.seed,
        ...
        "checkpoint_load": {
            ...
            "structured": structured_checkpoint,
        },
        ...
        "storage": {
            ...
            "structured_residual_cnl": structured_storage,
        },
        "gate": {
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Structured residual CNL-LUT-LIF architecture probe")
    ...
    p.add_argument("--delta-l2-weight", type=float, default=1e-3)
    ...
    p.add_argument("--learn-norm-affine", action=argparse.BooleanOptionalAction, default=False)
